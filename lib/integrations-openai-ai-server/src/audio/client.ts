...
/** Normalize speech audio to a compact, mono MP3 for private retention. */
export async function compressToMp3(audioBuffer: Buffer): Promise<Buffer> {
  const inputPath = join(tmpdir(), `input-${randomUUID()}`);
 ...
 */
export async function speechToTextWithSpeakers(
  audioBuffer: Buffer,
  format: "wav" | "mp3" | "webm" = "wav",
  prompt?: string,
): Promise<string> {
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: prompt ?? "Transcribe the recording. Put every speaker turn on its own line as `Speaker A: words spoken`. Use Speaker A, Speaker B, and so on consistently. Do not assign people or roles.",
      },
      {
        role: "user",
        content: [{ type: "input_audio", input_audio: { data: audioBase64, format } }],
      } as any,
    ] as any,
  });
