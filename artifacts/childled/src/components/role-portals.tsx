...
export function ParentPortal({
  ...
  const recentPhrases = gestalts
    .filter(
      (gestalt) =>
        gestalt.dateAdded && new Date(gestalt.dateAdded).getTime() >= weekAgo,
    )
    .slice(0, 3);
  const visibleRecentPhrases = recentPhrases.length
    ? recentPhrases
    : gestalts.slice(0, 3);
  const approvedInterests = interests.filter(
    (interest) => interest.status === "approved",
  );
  const parentSuggestions = interests.filter(
    (interest) => interest.status === "suggested",
  );
  const supports = child.sensorySupports?.length
  ...
  return (
    <div className="space-y-8 animate-rise delay-1">
      <section className="relative overflow-hidden rounded-3xl bg-secondary p-4 text-secondary-foreground soft-shadow sm:p-6 md:p-8">
        ...
        <div className="relative z-10 flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
          <div className="max-w-2xl">
            ...
            <h2 className="serif text-3xl font-semibold leading-tight text-foreground md:text-4xl">
              {child.name}'s communication map
            </h2>
            <p className="mt-3 text-sm leading-relaxed text-muted-foreground">
              A simple place to notice what your child is saying, what helps,
              and what you want to share with the care team.
            </p>
          </div>
          <div className="flex w-full min-w-0 flex-col gap-3 md:w-auto md:min-w-[210px]">
            <button
              onClick={onAddObservation}
              className="gold-action focus-ring inline-flex min-h-11 items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold text-primary"
            >
              <Plus size={16} /> Log observation
            </button>
            <button
              onClick={onViewPassport}
              className="focus-ring inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-border bg-card px-4 py-2.5 text-sm font-semibold text-foreground hover:bg-secondary/70"
            >
            ...
            <button
              ...
              className="focus-ring inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-border bg-card px-4 py-2.5 text-sm font-semibold text-foreground hover:bg-secondary/70"
            >
              <MessageCircle size={16} /> Ask the team
            </button>
          </div>
        </div>
      </section>
      ...
      <section
        className="relative overflow-hidden rounded-3xl bg-primary p-4 text-primary-foreground soft-shadow sm:p-6 md:p-8"
        data-testid="parent-weekly-highlights"
      >
      ...
      <div className="grid gap-6 lg:grid-cols-[1.15fr_.85fr]">
        <section
          ...
          data-testid="parent-recent-progress"
        >
          ...
          {visibleRecentPhrases.length ? (
            <div className="space-y-3">
              {visibleRecentPhrases.map((g) => (
                <div
                  key={g.id}
                  className="rounded-2xl border border-border bg-secondary/20 p-4"
                >
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <p className="serif text-xl font-semibold">“{g.phrase}”</p>
                    <span className="text-[11px] text-muted-foreground">
                      {g.dateAdded
                        ? new Date(g.dateAdded).toLocaleDateString("en-US", {
                            month: "short",
                            day: "numeric",
                          })
                        : "Recently"}
                  ...
                  <p className="mt-2 text-xs text-muted-foreground">
                    ...
                    {g.contexts?.[0] ?? "Shared with the team"}
                  </p>
                  <p className="mt-1 text-xs italic text-muted-foreground">
                    {g.source.toLocaleLowerCase().includes("review pending")
                      ? "Meaning is waiting for clinician review."
                      : "Meaning shown from the reviewed shared dictionary."}
                  </p>
                </div>
              ))}
            </div>
            ...
            <p className="rounded-2xl border border-dashed border-border p-5 text-sm text-muted-foreground">
              New phrases will appear here as your family and care team notice
              them.
            </p>
          )}
        </section>
        <section className="rounded-3xl border border-border bg-card p-6 soft-shadow md:p-8">
          <div className="mb-5 flex items-center gap-2">
            ...
            <h2 className="serif text-2xl font-semibold">This week's ideas</h2>
          </div>
          <div className="space-y-3">
            {[
              "Try joining your child’s interests.",
              "Pause and wait for communication.",
              "Repeat favorite phrases naturally.",
              "Model new language during play.",
            ].map((idea) => (
              <div
                key={idea}
                className="flex items-start gap-3 rounded-2xl bg-accent/5 p-3"
              >
                <span className="mt-0.5 grid size-5 place-items-center rounded-full bg-accent/20 text-primary">
                  ✓
                </span>
                <p className="text-sm leading-6">{idea}</p>
              </div>
            ))}
          </div>
          <p className="mt-4 text-xs leading-5 text-muted-foreground">
            These are everyday invitations, not instructions. Follow your
            child's lead.
          </p>
        </section>
      </div>
      ...
      >
        <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Add what you notice without changing a clinician-approved meaning.
            </p>
          </div>
          <button
            onClick={onAddObservation}
            className="focus-ring inline-flex items-center gap-2 rounded-xl border border-primary/20 bg-primary/5 px-3 py-2.5 text-xs font-semibold text-primary"
          >
        ...
        {gestalts.length ? (
            ...
            {gestalts.map((g) => (
              ...
              >
                ...
                <p className="mt-2 text-xs text-muted-foreground">
                  ...
                  {g.contexts?.join(", ") || "Not yet shared"}
                </p>
                <div className="mt-4 flex flex-wrap gap-2 border-t border-border pt-3">
                  <button
                    onClick={() => onPhraseNote(g, "meaning")}
                    className="focus-ring rounded-lg bg-secondary px-2.5 py-1.5 text-[11px] font-semibold text-primary hover:bg-secondary/70"
                  >
                    + Add meaning
                  </button>
                  <button
                    onClick={() => onPhraseNote(g, "context")}
                    className="focus-ring rounded-lg bg-secondary px-2.5 py-1.5 text-[11px] font-semibold text-primary hover:bg-secondary/70"
                  >
                    + Add context
                  </button>
          ...
          <p className="rounded-2xl border border-dashed border-border p-5 text-sm text-muted-foreground">
            Your shared dictionary is ready for its first phrase.
          </p>
        )}
      </section>
      ...
      <section
        className="rounded-3xl border border-border bg-card p-6 soft-shadow md:p-8"
        data-testid="parent-language-journey-preview"
      >
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
              {timeline.length
                ? `Over time, ${child.name} is beginning to share favorite phrases in new situations. New communication patterns have been noticed and added to the map.`
                : `Your child's communication story will grow here as the team notices new patterns over time.`}
            </p>
          </div>
          <Link
        ...
        {observations.length ? (
          <div className="space-y-3">
            {observations.slice(0, 3).map((observation) => (
              <div
                key={observation.id}
                className="rounded-2xl bg-secondary/25 p-4"
              >
                <p className="text-sm leading-6">{observation.body}</p>
                <p className="mt-1 text-[11px] text-muted-foreground">
                  {observation.context} ·{" "}
                  {new Date(observation.createdAt).toLocaleDateString("en-US", {
                    month: "short",
                    day: "numeric",
                  })}
                </p>
          ...
          <p className="rounded-2xl border border-dashed border-border p-5 text-sm text-muted-foreground">
            A small note about a phrase, context, question, or moment can help
            the team connect the dots.
          </p>
        )}
      </section>
    </div>
...
export function TeacherPortal({
  ...
  const [activeWorkspaceSection, setActiveWorkspaceSection] =
    useState("student-overview");
  const manualSectionSelectionUntil = useRef(0);
  const approvedInterests = interests.filter(
    (interest) => interest.status === "approved",
  );
  const suggestions = interests.filter(
    (interest) => interest.status === "suggested",
  );
  const unreviewedGestalts = gestalts.filter(
    (gestalt) => !isTeacherReviewedPhrase(gestalt),
  );
    ...
    () =>
      normalizedPhraseSearch
        ? gestalts.filter((gestalt) =>
            [gestalt.phrase, gestalt.meaning, gestalt.function].some((value) =>
              value.toLocaleLowerCase().includes(normalizedPhraseSearch),
            ),
          )
        : gestalts,
  ...
  return (
    <div className="space-y-8 animate-rise delay-1">
      <section
        id="student-overview"
        className="scroll-mt-28 overflow-hidden rounded-[2rem] border border-primary/20 bg-primary text-primary-foreground shadow-xl shadow-primary/10"
      >
        <div className="grid gap-8 p-6 md:p-8 lg:grid-cols-[1.15fr_.85fr]">
          <div>
            <div className="mb-4 flex items-center gap-4">
              <span className="grid size-16 shrink-0 place-items-center rounded-2xl bg-primary-foreground/10 text-xl font-bold ring-1 ring-primary-foreground/20">
                {child.name.charAt(0)}
              </span>
              <div>
                <p className="mono text-[10px] font-bold uppercase tracking-[0.2em] text-accent">
                  Student workspace
                </p>
                <h1 className="serif mt-1 text-4xl font-semibold leading-tight md:text-5xl">
                  {child.name}
                </h1>
            ...
            <p className="max-w-2xl text-sm leading-6 text-primary-foreground/75">
              Communication supports, shared understanding, and classroom
              context for {child.name}—all in one place.
            </p>
            <div className="mt-5 flex flex-wrap gap-2">
              {child.aacSnapshot?.isUser && (
      ...
      <div
        className="sticky top-14 z-10 -mx-2 rounded-2xl border border-primary/15 bg-background/95 p-2 shadow-[0_16px_36px_-28px_hsl(var(--brand-forest-950)/.75)] backdrop-blur-xl md:mx-0 md:p-3"
        data-testid="teacher-student-sticky-navigation"
      >
        <div className="flex items-center gap-3 px-2 pb-2 md:px-3">
          <span className="grid size-8 shrink-0 place-items-center rounded-xl bg-primary text-xs font-bold text-primary-foreground">
            {child.name.charAt(0)}
          </span>
          ...
          <span className="mono ml-auto hidden text-[9px] font-bold uppercase tracking-[.16em] text-primary sm:block">
            Student profile
          </span>
        </div>
        <nav
          aria-label={`${child.name} workspace sections`}
          className="flex max-w-full gap-1.5 overflow-x-auto rounded-xl bg-secondary/55 p-1.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
        >
      ...
      <section
        ...
        className="scroll-mt-52 rounded-3xl border border-border bg-card p-6 soft-shadow md:p-8"
      >
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Search only {child.name}’s reviewed phrase dictionary.
            </p>
          </div>
          <button
            onClick={onAddPhrase}
            className="gold-action focus-ring inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold text-primary"
          >
            <Plus size={15} /> Log New Phrase
          </button>
        </div>
        <label className="relative mt-6 block">
          <span className="sr-only">
            Search {child.name}’s phrase dictionary
          </span>
          <BookOpen
            size={18}
          ...
          <input
            ...
            className="focus-ring h-13 w-full rounded-2xl border border-border bg-background py-3 pl-12 pr-4 text-sm"
          />
        </label>
        {reviewedGestalts.length > 0 ? (
          <div className="mt-6 grid gap-3 lg:grid-cols-2">
            {reviewedGestalts.slice(0, phraseSearch ? 8 : 4).map((g) => (
              <article
                key={g.id}
                className="rounded-2xl border border-border bg-secondary/10 p-4"
              >
                ...
                <p className="mt-2 text-xs text-muted-foreground">
                  {g.contexts?.join(", ") || "Shared care-team context"}
                </p>
              </article>
            ))}
          </div>
          ...
          <p className="mt-6 rounded-2xl border border-dashed border-border p-5 text-sm text-muted-foreground">
            {phraseSearch
              ? `No reviewed meaning found for “${phraseSearch}” in ${child.name}’s dictionary.`
              : "No reviewed phrases have been shared yet."}
          </p>
        )}
      </section>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        >
          <div className="flex items-start justify-between gap-4">
            ...
            <button
              onClick={onAddObservation}
              className="focus-ring shrink-0 text-xs font-bold text-primary"
            >
              + Add Shared Moment
            </button>
          </div>
          {observations.length ? (
            ...
            <p className="mt-5 rounded-2xl border border-dashed border-border p-5 text-sm text-muted-foreground">
              Successful interactions, spontaneous phrases, AAC breakthroughs,
              and supports that worked will appear here.
            </p>
          )}
        </section>
        <section
        ...
        >
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h2 className="serif mt-2 text-2xl font-semibold">
                Recent care-team communication
              </h2>
            </div>
            <button
              onClick={onMessageTeam}
          ...
          {teamMessages.length ? (
            <div className="mt-5 space-y-3">
              {teamMessages.slice(0, 4).map((message) => (
                <article
                  key={message.id}
                  className="rounded-2xl bg-secondary/25 p-4"
                >
            ...
            <p className="mt-5 rounded-2xl border border-dashed border-border p-5 text-sm text-muted-foreground">
              Recent notes between teachers, clinicians, and family will appear
              here.
            </p>
          )}
        </section>
      ...
      <section id="aac-information" className="scroll-mt-52">
        <AacInformationCard childId={child.id} />
      </section>
      <section id="communication-profile" className="scroll-mt-52 space-y-6">
        <div className="rounded-3xl border border-border bg-card p-6 soft-shadow md:p-8">
          ...
          <h2 className="serif mt-2 text-2xl font-semibold">
            How {child.name} communicates
          </h2>
          <p className="mt-3 text-sm leading-7 text-muted-foreground">
            {child.communicationStyle ||
              "Communication preferences have not been added yet."}
          </p>
          <div className="mt-5 grid gap-3 sm:grid-cols-3">
            <div className="rounded-xl bg-secondary/35 p-4">
              <p className="mono text-[9px] font-bold uppercase tracking-wider text-muted-foreground">
                Reviewed phrases
              </p>
              <p className="serif mt-2 text-3xl font-semibold">
                {gestalts.length - unreviewedGestalts.length}
              </p>
            </div>
            <div className="rounded-xl bg-secondary/35 p-4">
              <p className="mono text-[9px] font-bold uppercase tracking-wider text-muted-foreground">
      ...
      <section id="classroom-supports" className="scroll-mt-52 space-y-5">
        <div>
          ...
          <h2 className="serif mt-2 text-3xl font-semibold">
            Support communication in the moment
          </h2>
          <p className="mt-2 text-sm text-muted-foreground">
            Use reviewed language and shared context while keeping the care team
            connected.
          </p>
        </div>
        <TeacherCommunicationHelper
          ...
          onLogObservation={onHelperLogObservation}
          onAddPhrase={onHelperAddPhrase}
          onAskTeam={onHelperAskTeam}
          onNotifyClinician={onHelperNotifyClinician}
          onSaveClassroomNote={onSaveClassroomNote}
          onMarkHeard={onMarkHeard}
          markingHeard={markingPhraseHeard}
