...
export function ClinicianUnclearSpeechPage({
  childId,
  child,
}: {
  childId?: number;
  ...
  const {
    ...
    {
      query: {
        queryKey: getListUnclearVocalizationsQueryKey({
          childId: childId ?? 0,
        }),
  ...
  return (
    <div className="space-y-10">
      ...
      <section
        className="rounded-2xl border border-primary/15 bg-secondary/35 p-5"
        data-testid="unclear-speech-safety-note"
      >
        <p className="text-sm font-semibold text-primary">
          Clinician-led review only
        </p>
        <p className="mt-1 text-sm leading-6 text-muted-foreground">
          Provider transcript text stays unchanged and separate from clinician
          interpretation. A cross-session label organizes this review page only;
          it never adds a phrase to the communication dictionary.
        </p>
      </section>
...
function UnclearSpeechGroupCard({
  childId,
  group,
}: {
  childId: number;
  group: UnclearVocalizationGroup;
}) {
    ...
    mutation: {
      onSuccess: (updatedReview) => {
        ...
        queryClient.setQueryData(
          getListUnclearVocalizationsQueryKey({ childId }),
          updatedReview,
        );
      },
      onError: () => {
        ...
        void queryClient.invalidateQueries({
          queryKey: getListUnclearVocalizationsQueryKey({ childId }),
        });
      },
    },
  });
  ...
  return (
    <article
      data-testid={`unclear-speech-group-${group.id}`}
      className="brand-card flex flex-col overflow-hidden rounded-3xl border border-border bg-card shadow-sm"
    >
      <div className="border-b border-border/50 bg-secondary/30 p-4 sm:p-5 md:px-7 md:py-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="max-w-xl">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-medium text-muted-foreground">
                ...
                {group.occurrenceCount === 1 ? "" : "s"} across{" "}
          ...
          <div className="flex w-full min-w-0 flex-col items-stretch sm:w-auto sm:min-w-[240px] sm:items-end">
            {!isEditing ? (
              <div className="flex items-center gap-3 bg-background rounded-xl border border-border p-2 pr-4 w-full justify-between">
                <div className="flex items-center gap-2 px-2">
                  ...
                  <span
                    className={`text-sm font-medium ${group.label ? "text-primary" : "text-muted-foreground italic"}`}
                  >
                    {group.label || "None"}
                  </span>
                </div>
              ...
              <div className="flex flex-col gap-2 w-full">
                <input
                  ...
                  className="h-11 w-full rounded-xl border border-input bg-background px-3 text-base outline-none transition-shadow focus-ring sm:h-10 sm:text-sm"
                  autoFocus
                ...
                <div className="flex gap-2 justify-end w-full">
                  ...
                  {group.label && (
                    <Button
                      ...
                      disabled={updateLabelMutation.isPending}
                      className="py-1.5 h-8 text-destructive border-destructive/30 hover:bg-destructive/10"
                    >
                      Remove
                    </Button>
                  )}
                ...
                {saveConflict && (
                  <p className="text-xs leading-5 text-destructive">
                    These records changed after you opened them, or the save
                    failed. Review the refreshed labels before trying again.
                  </p>
                )}
              </div>
            )}
        ...
        <div className="grid gap-3">
          {group.occurrences.map((occurrence) => (
            <div
              ...
              className="rounded-2xl border border-primary/10 bg-background p-4 soft-shadow"
            >
              <div className="flex flex-wrap gap-4 items-start justify-between border-b border-border/50 pb-3 mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold">
                      {new Date(occurrence.sessionDate).toLocaleDateString(
                        undefined,
                        {
                          weekday: "long",
                          year: "numeric",
                    ...
                    <p className="text-xs text-muted-foreground">
                      Session {occurrence.sessionId} • Position{" "}
                      {occurrence.position}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <span className="rounded-full bg-accent/20 px-2 py-1 text-xs font-medium text-accent-foreground capitalize">
                    Intelligibility:{" "}
                    {occurrence.intelligibility.replace("_", " ")}
                  </span>
              ...
              <div className="mt-4 border-t border-border/30 pt-4">
                ...
                {occurrence.audioClipUrl ? (
                  <audio
                    controls
                    preload="metadata"
                    className="h-11 w-full"
                    src={occurrence.audioClipUrl}
                    data-testid={`audio-unclear-clip-${occurrence.segmentId}`}
                  />
                ) : (
                  <p className="text-xs leading-5 text-muted-foreground">
                    No short clip is available because this transcript moment
                    did not include reliable timing. The full recording was not
                    retained after finalization.
                  </p>
                )}
              </div>
