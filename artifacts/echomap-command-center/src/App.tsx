  ...
  return (
    <main className="grid min-h-[100dvh] place-items-center bg-background p-6">
      <section className="cc-card w-full max-w-md rounded-3xl p-8 text-center">
        ...
        <p data-testid="auth-loading-message" className="mt-3 text-sm leading-6 text-muted-foreground">EchoMap is verifying your account and care-team access.</p>
      </section>
    </main>
  );
}
  ...
  return (
    <main className="grid min-h-[100dvh] place-items-center bg-background px-5 py-10">
      <section className="cc-card w-full max-w-2xl rounded-[2rem] p-8 text-center md:p-12">
        ...
        <p className="cc-mono mt-6 text-[10px] font-bold uppercase tracking-[.2em] text-muted-foreground">EchoMap Command Center</p>
...
function ClerkProviderWithRoutes() {
  ...
  return (
    <ClerkProvider publishableKey={clerkPubKey} proxyUrl={clerkProxyUrl} appearance={clerkAppearance} signInUrl={`${basePath}/sign-in`} signUpUrl={`${basePath}/sign-up`} localization={{ signIn: { start: { title: 'Welcome back to EchoMap', subtitle: 'Sign in to your verified care-team account' } }, signUp: { start: { title: 'Join EchoMap', subtitle: 'Create your verified care-team account' } } }} routerPush={(to) => setLocation(stripBase(to))} routerReplace={(to) => setLocation(stripBase(to), { replace: true })}>
