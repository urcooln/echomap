...
const viewerFrom = (req: Request): CareTeamActor | null =>
  req.childledAuthFailure ? null : effectiveViewerFromRequest(req);
const realViewerFrom = (req: Request): CareTeamActor | null =>
  req.childledAuthFailure ? null : realViewerFromRequest(req);
...
const developmentDemoSecurityOverview = () => ({
  ...
  recentConsentRecords: [
    {
      childId: 1,
      confirmedBy: "ChildLed Demo Administrator",
...
const reviewedSoapNoteFor = async (
  ...
  const generated: SoapNoteContent = {
    ...
    nlaObservations: [
      ...
      "ChildLed does not assign an NLA stage; clinicians review observed language in context.",
    ].join("\n"),
    gestaltTracking:
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        'attachment; filename="ChildLed-Parent-Coaching-Handbook.pdf"',
      );
...
type ChildLedTransaction = Parameters<Parameters<typeof db.transaction>[0]>[0];
...
const insertTransactionalAudit = (
  transaction: ChildLedTransaction,
  actor: ResolvedCareTeamActor,
  input: {
    action: string;
    targetType: string;
...
const calibrationDurationMilliseconds = async (
  ...
  const filePath = path.join(
    tmpdir(),
    `childled-calibration-${randomUUID()}.${sourceExtension}`,
  );
...
router.post("/sessions/transcription/speakers", async (req, res) => {
  ...
  if (hasUnreviewedChildTurn) {
    return fail(
      res,
      "Confirm every below-threshold turn in a Child speaker cluster before ChildLed can create Child-only clinical evidence.",
    );
  }
