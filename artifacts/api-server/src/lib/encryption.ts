...
const encryptionKey = () => {
  const configured = process.env.ECHOMAP_DATA_ENCRYPTION_KEY;
  if (configured) {
    const key = /^[0-9a-f]{64}$/i.test(configured)
      ? Buffer.from(configured, "hex")
      : Buffer.from(configured, "base64");
    if (key.length !== 32) {
      throw new Error("ECHOMAP_DATA_ENCRYPTION_KEY must encode exactly 32 bytes.");
    }
    return key;
  }
  if (process.env.NODE_ENV === "production") {
    throw new Error("ECHOMAP_DATA_ENCRYPTION_KEY is required in production to store sensitive data.");
  }
  const sessionSecret = process.env.SESSION_SECRET;
  if (!sessionSecret) throw new Error("SESSION_SECRET is required to derive the development encryption key.");
  return createHash("sha256").update(`echomap-development-data-key:${sessionSecret}`).digest();
};
...
export const encryptionBoundary = {
  ...
  keySource: process.env.ECHOMAP_DATA_ENCRYPTION_KEY
    ? "ECHOMAP_DATA_ENCRYPTION_KEY"
    : "development key derived from SESSION_SECRET",
  productionReady: Boolean(process.env.ECHOMAP_DATA_ENCRYPTION_KEY),
};