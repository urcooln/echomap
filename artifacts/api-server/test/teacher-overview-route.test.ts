...
test("teacher overview stays teacher-only and assigned-student scoped", async () => {
  ...
  const slpUserId = `teacher-overview-slp-${suffix}`;
  ...
  await db.insert(childCareTeamMembershipsTable).values([
    { childId: assigned.id, userId, role: "teacher" },
    { childId: assigned.id, userId: slpUserId, role: "clinician" },
  ]);
  const [conversation] = await db
    ...
    .values({
      organizationId: organization.id,
      childId: assigned.id,
      participantKey: [slpUserId, userId].sort().join(":"),
      createdByUserId: slpUserId,
    })
    .returning();
  assert.ok(conversation);
  await db.insert(teamConversationParticipantsTable).values([
    { conversationId: conversation.id, userId, role: "teacher" },
    { conversationId: conversation.id, userId: slpUserId, role: "clinician" },
  ]);
  await db.insert(teamMessagesTable).values([
    {
      ...
      body: "Assigned question",
    },
    {
      organizationId: organization.id,
      childId: unassigned.id,
      senderUserId: slpUserId,
      senderRole: "clinician",
      messageType: "question",
      body: "Unassigned question",
    },
  ]);
  await db.insert(clinicalGestaltsTable).values([
    {
      ...
      childId: assigned.id,
      phrase: "All done",
      normalizedPhrase: "all done",
      meaning: "Finished with the current activity",
    ...
    },
    {
      ...
      childId: unassigned.id,
      phrase: "All done now",
      normalizedPhrase: "all done now",
      meaning: "Unassigned private meaning",
      communicationFunction: "Protest",
      contexts: ["School"],
      emotionalState: "Regulated",
      source: "Clinician review",
  ...
  try {
    ...
    assert.deepEqual(
      overview.children.map((child: { childName: string }) => child.childName),
      ["Assigned Student"],
    );
    assert.equal(overview.recentActivity.length, 2);
    assert.ok(
      overview.recentActivity.every(
        (item: { target: string }) => item.target === "Assigned Student",
      ),
    );
    const missingStudentLookup = await fetch(
      `${base}/teacher-phrase-lookup?query=all%20done`,
    );
    assert.equal(missingStudentLookup.status, 400);
    const unassignedLookup = await fetch(
      `${base}/teacher-phrase-lookup?childId=${unassigned.id}&query=all%20done`,
    );
    assert.equal(unassignedLookup.status, 403);
    const lookup = await fetch(
      `${base}/teacher-phrase-lookup?childId=${assigned.id}&query=all%20done`,
    );
    assert.equal(lookup.status, 200);
    const lookupResults = await lookup.json();
    assert.deepEqual(
      lookupResults.map((result: { childName: string }) => result.childName),
      ["Assigned Student"],
    );
    assert.equal(
      lookupResults[0].meaning,
    ...
    await db
      ...
      .where(eq(clinicalGestaltsTable.createdByUserId, userId));
    await db.delete(teamMessagesTable).where(
      inArray(
        teamMessagesTable.childId,
    ...
    await db
      ...
      .where(
        inArray(childCareTeamMembershipsTable.userId, [userId, slpUserId]),
      );
    ...
    await db
      ...
      .where(inArray(organizationMembershipsTable.userId, [userId, slpUserId]));
    await db
      .delete(usersTable)
      .where(inArray(usersTable.id, [userId, slpUserId]));
    await db
