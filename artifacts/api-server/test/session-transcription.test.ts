...
test("keeps distinct labeled speaker turns in transcript order", () => {
  ...
  assert.deepEqual(
    segments.map(({ confidence }) => confidence),
...
test("reassignment changes which turns are eligible for analysis", () => {
  ...
  const before = childSpeechChunks(
    segments,
    new Map([
      ["Speaker A", "slp"],
      ["Speaker B", "unassigned"],
    ]),
  );
  const after = childSpeechChunks(
    segments,
    new Map([
      ["Speaker A", "slp"],
      ["Speaker B", "child"],
...
test("creates neutral clinician-review turns when speaker grouping is unavailable", () => {
  ...
  assert.deepEqual(
    segments.map(
      ({ speakerLabel, text, position, confidence, intelligibility }) => ({
        speakerLabel,
        text,
        position,
        confidence,
...
test("keeps repeated transcript phrases as separate neutral review items", () => {
  const segments = manualTranscriptReviewSegments(
    "Testing, testing, testing.\nReady set go ready set go.",
  );

  assert.deepEqual(
    segments.map(({ text, position }) => ({ text, position })),
    [
      { text: "Testing", position: 0 },
      { text: "Testing", position: 1 },
      { text: "Testing", position: 2 },
      { text: "Ready set go", position: 3 },
      { text: "Ready set go", position: 4 },
    ],
  );
});

test("keeps phrase replay timestamps absolute across provider chunks", () => {
  const segments = timedTranscriptReviewSegments([
    {
      text: "First phrase.",
      offsetSeconds: 0,
      segments: [{ text: " First phrase. ", startSeconds: 1.2, endSeconds: 2.6 }],
    },
    {
      text: "Second phrase.",
      offsetSeconds: 10,
      segments: [{ text: "Second phrase.", startSeconds: 0.4, endSeconds: 1.1 }],
    },
  ]);

  assert.deepEqual(
    segments.map(({ text, position, startTimeMilliseconds, durationMilliseconds }) => ({
      text,
      position,
...
test("gives repeated phrases separate word-level replay ranges", () => {
  const segments = timedTranscriptReviewSegments([
    {
      ...
      words: [
        { text: "Testing", startSeconds: 0.8, endSeconds: 1.3 },
        { text: "testing", startSeconds: 1.7, endSeconds: 2.2 },
        { text: "testing", startSeconds: 2.7, endSeconds: 3.4 },
      ],
    },
  ]);

  assert.deepEqual(
    segments.map(({ text, startTimeMilliseconds, durationMilliseconds }) => ({
      text,
      startTimeMilliseconds,
      durationMilliseconds,
    })),
    [
      { text: "Testing", startTimeMilliseconds: 5800, durationMilliseconds: 500 },
      { text: "Testing", startTimeMilliseconds: 6700, durationMilliseconds: 500 },
      { text: "Testing", startTimeMilliseconds: 7700, durationMilliseconds: 700 },
    ],
  );
});
...
test("derives durable session review counts and remaining work", () => {
  assert.deepEqual(
    ...
    ),
    {
      ...
      unsure: 1,
      unintelligible: 1,
      nextStep: "Continue reviewing Child language.",
    },
  );
});
...
test("uses persisted review and phrase inbox thresholds for the next step", () => {
  assert.equal(
    sessionReviewProgressFor(0, [], 0).nextStep,
    "Continue reviewing Child language.",
  );
  assert.equal(
    sessionReviewProgressFor(3, ["pending"], 1).nextStep,
    "Continue reviewing Child language.",
  );
  assert.equal(
  ...
  assert.equal(
    sessionReviewProgressFor(2, ["not_child", "unintelligible"], 0).nextStep,
    "Review complete. Continue to the session summary.",
  );
});

test("review progress does not depend on speaker identification state", () => {
  const unavailableSpeakerState = sessionReviewProgressFor(
    2,
    ["child", "pending"],
    0,
  );
  const identifiedSpeakerState = sessionReviewProgressFor(
    2,
    ["child", "pending"],
    0,
  );
...
test("creates actionable provisional candidates without treating them as Child evidence", () => {
  ...
  const recurring = candidates.find(
    (candidate) => candidate.normalizedPhrase === "blast off",
  );
  const partial = candidates.find(
    (candidate) => candidate.normalizedPhrase === "more",
  );
  ...
  assert.equal(
    candidates.some((candidate) =>
      candidate.normalizedPhrase.includes("unclear"),
    ),
    false,
  );
});
...
test("exposes the configured diarization provider without assigning clinical roles", async () => {
  const { speakerDiarizationAvailability } =
    await import("../src/lib/speaker-diarization");

  assert.equal(speakerDiarizationAvailability.status, "available");
  assert.equal(
    speakerDiarizationAvailability.provider,
    "Replit OpenAI audio diarization",
  );
});
...
test("keeps raw transcription separate from alternating speaker turns", () => {
  ...
  const result = buildTranscriptResult(
    ["Blast off! What would you like to do? Again!"],
    8.1,
    separatedSegments,
  );
...
test("preserves structured diarization confidence for every turn", () => {
  ...
  assert.deepEqual(
    separatedSegments.map(({ speakerLabel, text, confidence }) => [
      speakerLabel,
      text,
      confidence,
    ]),
...
test("preserves explicit unintelligible and partial vocal turns without guessing words", () => {
  const segments = segmentSeparatedSpeakerTranscript(
    JSON.stringify({
      turns: [
        {
          speaker: "Speaker A",
...
test("normalizes provider uncertainty markers conservatively", () => {
  assert.equal(
    intelligibilityFrom(undefined, "[unintelligible]"),
    "unintelligible",
  );
  assert.equal(
    intelligibilityFrom(undefined, "more [unclear]"),
...
test("diarization provider parsing cannot downgrade explicit uncertainty markers", () => {
  const segments = parseProviderTurns(
    JSON.stringify({
      turns: [
        {
          speaker: "Speaker A",
...
test("uncertainty annotation tokens never become phrase evidence text", () => {
  ...
  assert.equal(
    segmentTranscript([supportedText]).some((phrase) =>
      phrase.phrase.includes("unclear"),
    ),
    false,
  );
});
...
test("keeps opaque characteristics and numeric confidence for protected profile matching", () => {
  ...
  const [segment] = segmentSeparatedSpeakerTranscript(
    JSON.stringify({
      turns: [
        {
...
test("rejects structured diarization without a supported confidence tier", () => {
  assert.deepEqual(
    segmentSeparatedSpeakerTranscript(
      JSON.stringify({
        turns: [
          { speaker: "Speaker A", text: "Blast off!", confidence: "certain" },
        ],
      }),
    ),
    [],
...
test("keeps speaker separation available after a delay longer than the former six-second window", async () => {
  ...
  assert.equal(
    persisted,
    false,
    "scheduling must return before separation resolves",
  );
...
test("requires more than a protected profile match for a provisional role", () => {
  const [inference] = inferSpeakerRoles({
    ...
    profiles: [
      { profileSignatureHash: "opaque-child-scoped-profile", role: "child" },
    ],
    confirmedChildPatterns: [],
    learning: [],
  });
...
test("uses multiple bounded signals for provisional review guidance", () => {
  const [inference] = inferSpeakerRoles({
    ...
    learning: [
      { role: "child", featureKey: "confirmed_role", confirmedCount: 3 },
    ],
  });
  ...
  assert.ok(
    inference?.signalSummary.some(
      (signal) =>
        signal.signal === "confirmed_child_patterns" && signal.contribution > 0,
    ),
  );
});
...
test("groups recurring phrase fragments across separate child turns", () => {
  ...
  assert.equal(
    candidates.find(
      (candidate) => candidate.normalizedPhrase === "want more bubbles",
    )?.frequency,
    2,
  );
});
...
test("rejects a provider byte limit that would require too many chunks", async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      ...
      "code" in error &&
      error.code === "TRANSCRIPTION_PROVIDER_LIMIT",
  );
});
