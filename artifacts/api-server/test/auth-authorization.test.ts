...
test("unauthenticated and expired sessions cannot use protected child data", () => {
  ...
  assert.equal(
    hasVerifiedCareTeamSession({ ...parent, expiresAt: 9_000 }, 9_000),
    false,
  );
  assert.equal(
    canAccessAssignedChild({ ...parent, expiresAt: 9_000 }, 11),
    false,
  );
});
...
test("child access is limited to explicitly assigned memberships", () => {
  ...
  assert.equal(
    canAccessAssignedChild({ ...parent, role: "Administrator" }, 12),
    false,
  );
});
...
test("a Clerk account needs a verified email before its local profile is considered", () => {
  assert.equal(
    hasVerifiedEmailAddress([{ verification: { status: "unverified" } }]),
    false,
  );
  assert.equal(
    hasVerifiedEmailAddress([{ verification: { status: "verified" } }]),
    true,
  );
});
...
test("Super Admin capability is limited to explicit owners and the enabled demo", () => {
  assert.equal(
    isSuperAdminIdentity({
      userId: "owner-1",
      isAdmin: true,
      configuredUserIds: "owner-1, owner-2",
    }),
    true,
  );
  assert.equal(
